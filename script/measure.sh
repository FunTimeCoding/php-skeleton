...
if [ "${1}" = --ci-mode ]; then
    ...
    if [ -f "${HOME}/.sonar-qube-tools.sh" ]; then
        ...
        sonar-scanner "-Dsonar.projectKey=${PROJECT_NAME}" -Dsonar.sources=. "-Dsonar.host.url=${SONAR_SERVER}" "-Dsonar.login=${SONAR_TOKEN}" '-Dsonar.exclusions=build/**,tmp/**,vendor/**' | "${TEE}" build/log/sonar-runner.log
    else
