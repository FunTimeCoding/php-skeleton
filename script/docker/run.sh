...
if [ "${FOUND}" = false ]; then
    ...
    if [ "${DEVELOPMENT}" = true ]; then
        docker create --name php-skeleton --volume "${WORKING_DIRECTORY}:/php-skeleton" funtimecoding/php-skeleton
    else
    ...
    #docker create --name php-skeleton --volume "${WORKING_DIRECTORY}:/php-skeleton" --entrypoint /php-skeleton/bin/other.sh funtimecoding/php-skeleton
